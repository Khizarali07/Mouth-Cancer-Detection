...
const { getCurrentUser } = require("@/lib/actions/userActions");
...
export const getFiles = async ({
  ...
  try {
    ...
    if (!currentUser) {
      console.log("No current user found in getFiles");
...
export const getFileById = async (fileId) => {
  ...
  try {
    ...
    // Fetch the file using the fileId passed as a parameter.
    const file = await databases.getDocument(
      appwriteConfig.databaseId, // The database ID in Appwrite.
      appwriteConfig.filesCollectionId, // The collection ID for files.
      fileId // The ID of the file document.
    );
    ...
    // Return a fallback object to ensure the application continues working
    return {
      error: true,
...
export const updateData = async ({
  ...
  try {
    if (step === 1) {
      ...
      let extension = currentFile?.extension?.split(",");
      extension[0] = getFileType(bucketFile.name)?.extension;
      ...
      try {
        await storage.deleteFile(
          appwriteConfig.bucketId,
          currentFile.biopsyBucketFileId
        );
      } catch (err) {
        if (err.code === 404) {
          console.warn(
      ...
      let extension = currentFile.extension?.split(",");
      extension[1] = getFileType(bucketFile.name)?.extension;

      extension = extension?.join(",");

      console.log("000111 :", extension);
      ...
      updatedFile = await databases.updateDocument(
        ...
        fileId, // ID of the document to be updated
        {
          urlBiopsy: constructFileUrl(bucketFile.$id), // URL for accessing the file
          extension: extension, // File extension
          size: currentFile.size + bucketFile.sizeOriginal, // File size
          resultBiopsy: Result,
          biopsyBucketFileId: bucketFile.$id, // File ID in the bucket
        }
      );
    } else if (step === 3) {
      // // If the document deletion was successful, delete the actual file from storage
...
export async function getTotalSpaceUsed() {
  try {
    ...
    const currentUser = await getCurrentUser();
    if (!currentUser) {
      console.log("No current user found in getTotalSpaceUsed");
    ...
    const totalSpace = {
      ...
      latestDate: currentUser.$updatedAt,
      used: 0,
