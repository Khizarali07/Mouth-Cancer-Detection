...
export const getAdminStats = async () => {
  try {
    ...
    // Get message stats (only if messages collection is configured)
    let messageStats = {
      total: 0,
      pending: 0,
      completed: 0,
    ...
    try {
      if (appwriteConfig.messagesCollectionId) {
        const { getMessageStats } = await import("./messageActions");
        messageStats = await getMessageStats();
      }
    } catch (error) {
      console.log("Message stats not available:", error.message);
    }
