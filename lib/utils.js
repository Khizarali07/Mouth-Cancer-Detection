...
export const getUsageSummary = (totalSpace) => {
  // Function to calculate the latest date among all categories
  const getLatestDate = (categories) => {
    return Object.values(categories)
      .map((category) => category.latestDate)
      .filter((date) => date) // Exclude null or empty dates
      .sort((a, b) => new Date(b) - new Date(a))[0];
  };

  // Determine the latest date
  const latestDate = getLatestDate(totalSpace);
  return [
    {
      title: "Documents",
      size: totalSpace.used,
    ...
    },
    {
      ...
      icon: "/assets/icons/card-setting.svg",
...
export function formatDataForBackend(formData) {
  return {
    Age: formData.age,
    Country: capitalize(formData.country), // "india" → "India"
    Gender: capitalize(formData.gender),
    "Tobacco Use": capitalize(formData.tobaccoUse),
    "Alcohol Consumption": capitalize(formData.alcoholUse),
    "HPV Infection": capitalize(formData.hpvInfection),
    "Betel Quid Use": capitalize(formData.betelQuidUse),
    "Chronic Sun Exposure": capitalize(formData.chronicSunExposure),
    "Poor Oral Hygiene": capitalize(formData.poorOralHygiene),
    "Diet (Fruits & Vegetables Intake)": capitalize(formData.dietIntake),
    "Family History of Cancer": capitalize(formData.familyHistory),
    "Compromised Immune System": capitalize(formData.compromisedImmuneSystem),
    "Oral Lesions": capitalize(formData.oralLesions),
    "Unexplained Bleeding": capitalize(formData.unexplainedBleeding),
    "Difficulty Swallowing": capitalize(formData.difficultySwallowing),
    "White or Red Patches in Mouth": capitalize(formData.whitePatches),
    "Cancer Stage": formData.cancerStage,
    "Treatment Type": capitalize(formData.treatmentType),
    "Early Diagnosis": capitalize(formData.earlyDiagnosis),
    "Tumor Size (cm)": formData.tumorSize,
    "Survival Rate (5-Year, %)": formData.survivalRate,
    "Cost of Treatment (USD)": formData.treatmentCost,
    "Economic Burden (Lost Workdays per Year)": formData.economicBurden,
  };
}
